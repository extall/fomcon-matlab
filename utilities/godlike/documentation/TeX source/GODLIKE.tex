...
objective function \texttt{obj\_function}. The upper and lower bounds
in each dimension is set by the arguments
\texttt{lb} and \texttt{ub}.
\\ \\
\subsubsection*{Output Arguments}
\begin{description}
\item[\texttt{solution}] the solution vector returned. It is always a
row-vector of size [1 $\times$ dimensions].

\item[\texttt{function\_value}] the function value associated with the
...
\begin{description}
\item[\texttt{obj\_function}] The user-provided objective function
...
\item[\texttt{lb \& ub}] In case all bounds are the same for every
...
GODLIKE(obj\_function, lb, ub, confcn, 'option',
'value', ...)}
\end{quote}
...
\texttt{options = set\_options('option', 'value', ...)}\\
 \texttt{[solution, function\_value, exitflag, output] = ...\\
GODLIKE(obj\_function, lb, ub, confcn, options)}
\end{quote}
...
\begin{description}
\item[\texttt{confcn}] Constraint function(s); placeholder for future
implementation of constraint functions. Currently unused.
\item[\texttt{options} (or \texttt{('option', value)} pairs)] These set the options
...
\item[\texttt{exitflag}] A simple value which informs the user of the
...
\item[\texttt{output}] A structure that contains the following fields:
         \begin{description}

         \item[\texttt{algorithms}] lists the algorithms used in the
         optimization.

         \item[\texttt{message}] A message that contains a description of
         the reason GODLIKE terminated.

         \item[\texttt{algorithm\_info}] A structure that contains more
         ...
         \item [\texttt{funcCount}] total number of function evaluations
         made.

         \item [\texttt{iterations}] total number of GODLIKE iterations
         made.
...
\begin{quote}
 ...
 exitflag, output] = ...\\
GODLIKE(\{obj\_function1, obj\_function2,\ldots\}, lb,
ub, options)}
\end{quote}
...
\begin{quote}
 ...
 Pareto\_individuals, Pareto\_fitnesses,...\\
 exitflag, output] = ...\\
GODLIKE(obj\_function, lb, ub, options)}
\end{quote}
